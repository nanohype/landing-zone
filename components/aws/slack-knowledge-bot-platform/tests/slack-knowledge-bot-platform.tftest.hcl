# Unit tests for slack-knowledge-bot-platform's audit-bucket in-transit hardening.
# The audit archive holds per-query audit records beyond the DDB TTL window; this
# component owns the bucket's resource policy, which seeds the DenyInsecureTransport
# baseline. The contract under test: the bucket policy denies every non-TLS request
# (s3:* when aws:SecureTransport=false). Drop that statement and the archive is
...
mock_provider "aws" {
  ...
  # The Aurora module's managed master-user secret is a computed nested list the
  # app-access policy and an output index into ([0].secret_arn); mock generates it
  # empty, so pin a single-element secret block.
  mock_resource "aws_rds_cluster" {
    defaults = {
      master_user_secret = [{
        kms_key_id    = "arn:aws:kms:us-west-2:123456789012:key/mock"
        secret_arn    = "arn:aws:secretsmanager:us-west-2:123456789012:secret:mock-rds"
