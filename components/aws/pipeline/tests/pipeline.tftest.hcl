...
# In adopt mode pipeline participates in a VPC it does not own. network-preflight.tf reads
# each private subnet and the cluster SG and asserts they reside in network.vpc_id. The
# overrides stand in for the AWS reads (data sources are gated on adopt mode, so create-mode
# runs above never execute them and need no mocks).
...
run "adopt_mode_plans_with_matching_placement" {
  ...
  assert {
    condition     = output.tenant_outputs["t1"].connector_policy_json != null
...
# reference to an SG in another VPC would attach an MSK ingress rule that silently never
# matches.
...
# AZ coverage is pipeline's own floor, checked in both modes: fewer than two distinct zones
# fails before MSK/Batch placement would.
