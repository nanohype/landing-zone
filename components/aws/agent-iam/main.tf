    ...
    Statement = [
      {
        ...
        # namespace: the app's chart pods and AgentFleet pods, both running as
        # the operator-created tenant-runtime SA bound to this role by a Pod
        # Identity association, their substrate access granted through the
        # operator-generated datastore-access + capability-access policies. The
        # ceiling therefore covers the union of
      ...
      {
        # EventBridge Scheduler targets fire through a scheduler-invoke role the
        # operator mints from spec.identity.capabilities; CreateSchedule requires
...
# through guardrails + ship logs/traces. Per-tenant extras (databases, buckets,
# queues, caches, streams, plus SES / EventBridge Scheduler) are generated by
# the operator from Platform.spec.datastores + spec.identity.capabilities as
# scoped inline policies, always under the boundary above.
################################################################################
...
resource "aws_iam_role_policy" "operator" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # Read + lifecycle the per-Platform EventBridge Scheduler invoke role
        # (<env>-<platform>-scheduler-invoke), minted for a Platform that declares
        # the eventBridgeScheduler capability. detachAndDeleteRole walks the
        # attached + inline policies before DeleteRole, so those list/get verbs are
        # required alongside the assume-policy update.
        Sid    = "SchedulerInvokeRoleRead"
        Effect = "Allow"
        Action = [
          "iam:GetRole",
          "iam:DeleteRole",
      ...
      {
        # Minting the invoke role is gated on the tenant permissions boundary,
        # exactly as tenant + session role creation is — the operator may never
        # mint a role that outranks the boundary. The operator generates the role's
        # trust (scheduler.amazonaws.com, scoped to the tenant's schedule prefix)
        # and its sqs:SendMessage inline policy from spec.identity.capabilities +
        # the tenant's queue datastores. The -scheduler-invoke name is what the
        # SchedulerInvokeRolePass tenant-boundary statement keys its PassRole
        # allow-list on, so the tenant can pass this role to the Scheduler service.
        Sid    = "SchedulerInvokeRoleWriteWithBoundary"
        Effect = "Allow"
        Action = [
          "iam:CreateRole",
          "iam:AttachRolePolicy",
