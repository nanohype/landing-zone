...
2. **Validate** the environment and component exist at `live/aws/<account>/us-east-1/$ARGUMENTS[0]/$ARGUMENTS[1]/`

3. **Show what will be destroyed**: run `terragrunt plan -destroy` in `live/aws/<account>/us-east-1/$ARGUMENTS[0]/$ARGUMENTS[1]/` and summarize the resources
...
6. Only after confirmation, run `task destroy ACCOUNT=<account> REGION=us-east-1 ENVIRONMENT=$ARGUMENTS[0] COMPONENT=$ARGUMENTS[1]`
